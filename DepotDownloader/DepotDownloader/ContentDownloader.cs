...
namespace DepotDownloader
{
    ...
    public static class ContentDownloader
    {
        ...
        public static bool InitializeSteam3WithToken(string username, string loginToken)
        {
            steam3 = new Steam3Session(
                new SteamUser.LogOnDetails
                {
        ...
        public static async Task DownloadPubfileAsync(uint appId, ulong publishedFileId)
        {
            ...
            else if (details?.hcontent_file > 0)
            {
                await DownloadAppAsync(
                    appId,
                    new List<(uint, ulong)> { (appId, details.hcontent_file) },
                    DEFAULT_BRANCH,
                    null,
                    null,
                    null,
        ...
        private static async Task DownloadWebFile(uint appId, string fileName, string url)
        {
            ...
            {
                if (!Config.Silent) Console.WriteLine("Downloading {0}", fileName);
        ...
        )
        {
            ...
            if (!await AccountHasAccess(appId, appId))
            {
                ...
                {
                    if (!Config.Silent) Console.WriteLine("Obtained FreeOnDemand license for app {0}", appId);
            ...
            {
                if (!Config.Silent) Console.WriteLine("Using app branch: '{0}'.", branch);
            ...
            if (!Config.Silent) Console.WriteLine();
        ...
        public static async Task<DepotDownloadInfo> GetDepotInfo(uint depotId, uint appId, ulong manifestId, string branch)
        {
            ...
            if (manifestId == INVALID_MANIFEST_ID)
            {
                ...
                if (manifestId == INVALID_MANIFEST_ID && !string.Equals(
                        branch,
                        DEFAULT_BRANCH,
                    ...
                    ))
                {
                    if (!Config.Silent)
                        Console.WriteLine(
                            "Warning: Depot {0} does not have branch named \"{1}\". Trying {2} branch.",
                            depotId,
                            branch,
                            DEFAULT_BRANCH
                        );
        ...
        {
            if (!Config.Silent) Ansi.Progress(Ansi.ProgressState.Indeterminate);
            ...
            if (!Config.Silent) Ansi.Progress(Ansi.ProgressState.Hidden);
        ...
        private static async Task<DepotFilesData> ProcessDepotManifestAndFiles(
        ...
        )
        {
            ...
            if (!Config.Silent) Console.WriteLine("Processing depot {0}", depot.DepotId);
            ...
            else
            {
                ...
                {
                    if (!Config.Silent) Console.WriteLine($"Downloading depot {depot.DepotId} manifest");
                    ...
                    do
                    {
                        ...
                        try
                        {
                            ...
                            if (steam3.CDNAuthTokens.TryGetValue(
                                    (depot.DepotId, connection.Host),
                        ...
                        {
                            Console.WriteLine(
                                "Connection timeout downloading depot manifest {0} {1}. Retrying.",
                                depot.DepotId,
                                depot.ManifestId
                            );
                        }
                        catch (SteamKitWebRequestException e)
                        {
                            ...
                            {
                                Console.WriteLine(
                                    "Encountered {2} for depot manifest {0} {1}. Aborting.",
                                    depot.DepotId,
                                    depot.ManifestId,
                                    (int)e.StatusCode
                                );
                            ...
                            {
                                Console.WriteLine(
                                    "Encountered 404 for depot manifest {0} {1}. Aborting.",
                                    depot.DepotId,
                                    depot.ManifestId
                                );
                                break;
                            ...
                            Console.WriteLine(
                                "Encountered error downloading depot manifest {0} {1}: {2}",
                                depot.DepotId,
                                depot.ManifestId,
                                e.StatusCode
                            );
                        ...
                        catch (Exception e)
                        {
                            cdnPool.ReturnBrokenConnection(connection);
                            Console.WriteLine(
                                "Encountered error downloading manifest for depot {0} {1}: {2}",
                                depot.DepotId,
                                depot.ManifestId,
                                e.Message
                            );
            ...
            if (!Config.Silent) Console.WriteLine("Manifest {0} ({1})", depot.ManifestId, newManifest.CreationTime);
            ...
            var filesAfterExclusions =
                newManifest.Files.AsParallel().Where(f => TestIsFileIncluded(f.FileName)).ToList();
        ...
        )
        {
            ...
            if (!Config.Silent) Console.WriteLine("Downloading depot {0}", depot.DepotId);

            var files = depotFilesData.filteredFiles.Where(f => !f.Flags.HasFlag(EDepotFileFlag.Directory)).ToArray();
            var networkChunkQueue =
                new ConcurrentQueue<(FileStreamData fileStreamData, DepotManifest.FileData fileData,
            ...
            if (depotFilesData.previousManifest != null)
            {
                ...
                foreach (var existingFileName in previousFilteredFiles)
                {
                    ...
                    File.Delete(fileFinalPath);
                    if (!Config.Silent) Console.WriteLine("Deleted {0}", fileFinalPath);
                }
            }
            ...
            if (!Config.Silent)
                Console.WriteLine(
                    "Depot {0} - Downloaded {1} bytes ({2} bytes uncompressed)",
                    depot.DepotId,
                    depotCounter.depotBytesCompressed,
        ...
        )
        {
            ...
            if (!fileDidExist)
            {
                if (!Config.Silent) Console.WriteLine("Pre-allocating {0}", fileFinalPath);
                ...
                {
                    throw new ContentDownloaderException(
                        string.Format("Failed to allocate file {0}: {1}", fileFinalPath, ex.Message)
                    );
                }
            ...
            else
            {
                ...
                if (oldManifestFile != null)
                {
                    ...
                    if (Config.VerifyAll || !hashMatches)
                    {
                        ...
                        if (Config.VerifyAll)
                        {
                            if (!Config.Silent) Console.WriteLine("Validating {0}", fileFinalPath);
                        }
                ...
                if (neededChunks.Count == 0)
                {
                    lock (depotDownloadCounter)
                    {
                        ...
                        Config.OnProgressCallback(
                            new DownloadConfig.ProgressEventInfo
                            {
                                bytesDownloaded = depotDownloadCounter.sizeDownloaded,
                                bytesToDownload = depotDownloadCounter.completeDownloadSize,
                                percentage = depotDownloadCounter.sizeDownloaded /
                                             (float)depotDownloadCounter.completeDownloadSize
                            }
                        );
        ...
        )
        {
            ...
            try
            {
                do
                {
                    ...
                    try
                    {
                        ...
                        if (!Config.Silent)
                            DebugLog.WriteLine(
                                "ContentDownloader",
                                "Downloading chunk {0} from {1} with {2}",
                                chunkID,
                                connection,
                                cdnPool.ProxyServer != null ? cdnPool.ProxyServer : "no proxy"
                            );
                    ...
                    catch (SteamKitWebRequestException e)
                    {
                        ...
                        if (e.StatusCode == HttpStatusCode.Forbidden &&
                            (!steam3.CDNAuthTokens.TryGetValue(
                                (depot.DepotId, connection.Host),
                                out var authTokenCallbackPromise
                            ) || !authTokenCallbackPromise.Task.IsCompleted))
                        {
                            await steam3.RequestCDNAuthToken(depot.AppId, depot.DepotId, connection);
                    ...
                    catch (Exception e)
                    {
                        cdnPool.ReturnBrokenConnection(connection);
                        Console.WriteLine(
                            "Encountered unexpected error downloading chunk {0}: {1}",
                            chunkID,
                            e.Message
                        );
                    }
                ...
                {
                    Console.WriteLine(
                        "Failed to find any server with chunk {0} for depot {1}. Aborting.",
                        chunkID,
                        depot.DepotId
                    );
                    cts.Cancel();
