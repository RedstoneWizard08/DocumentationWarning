...
namespace DepotDownloader
{
    class Steam3Session
    {
        ...
        public ConcurrentDictionary<(uint, string), TaskCompletionSource<SteamContent.CDNAuthToken>> CDNAuthTokens
        {
            get;
        } = [];
        ...
        public Steam3Session(SteamUser.LogOnDetails details)
        {
            ...
            if (!ContentDownloader.Config.Silent) Console.Write("Connecting to Steam3...");
            Connect();
        }
        ...
        public async Task RequestAppInfo(uint appId, bool bForce = false)
        {
            ...
            foreach (var appInfo in appInfoMultiple.Results)
            {
                foreach (var app_value in appInfo.Apps)
                {
                    ...
                    if (!ContentDownloader.Config.Silent) Console.WriteLine("Got AppInfo for {0}", app.ID);
                    AppInfo[app.ID] = app;
                }
        ...
        public async Task RequestDepotKey(uint depotId, uint appid = 0)
        {
            ...
            if (!ContentDownloader.Config.Silent)
        ...
        public async Task<ulong> GetDepotManifestRequestCodeAsync(
            uint depotId,
            uint appId,
            ulong manifestId,
            string branch
        ...
        public async Task CheckAppBetaPassword(uint appid, string password)
        {
            ...
            if (!ContentDownloader.Config.Silent)
                Console.WriteLine(
                    "Retrieved {0} beta keys with result: {1}",
                    appPassword.BetaPasswords.Count,
                    appPassword.Result
                );
        ...
        {
            if (!AppBetaPasswords.TryGetValue(
                    branch,
                    out var branchPassword
        ...
        public void Disconnect(bool sendLogOff = true)
        {
            ...
            if (!ContentDownloader.Config.Silent) Ansi.Progress(Ansi.ProgressState.Hidden);
        ...
        {
            if (!ContentDownloader.Config.Silent) Console.WriteLine(" Done!");
            bConnecting = false;
            ...
            if (!authenticatedUser)
            {
                if (!ContentDownloader.Config.Silent) Console.Write("Logging anonymously into Steam3...");
            ...
            else
            {
                ...
                {
                    if (logonDetails.Username != null && logonDetails.Password != null &&
                        logonDetails.AccessToken is null)
                    {
                        try
                        {
                            _ = AccountSettingsStore.Instance.GuardData.TryGetValue(
        ...
        private void DisconnectedCallback(SteamClient.DisconnectedCallback disconnected)
        {
            ...
            DebugLog.WriteLine(
                ...
                $"Disconnected: bIsConnectionRecovery = {bIsConnectionRecovery}, UserInitiated = {disconnected.UserInitiated}, bExpectingDisconnectRemote = {bExpectingDisconnectRemote}"
            );

            // When recovering the connection, we want to reconnect even if the remote disconnects us
            if (!bIsConnectionRecovery && (disconnected.UserInitiated || bExpectingDisconnectRemote))
            {
                if (!ContentDownloader.Config.Silent) Console.WriteLine("Disconnected from Steam");
